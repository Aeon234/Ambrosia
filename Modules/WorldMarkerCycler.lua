...
function WorldMarkerCycler:Placer_Init()
	if not _G["WorldMarker_Placer"] then
		...
		Placer_Button:HookScript("PostClick", function(self)
			...
			if not self:GetAttribute("enableMarkers") then
				PrintDebug("Marker placement disabled")
			else
				local MarkerNum
				for _, marker in ipairs(Ambrosia.WorldMarkers) do
					if marker.wmID == self:GetAttribute("WorldMarker_Previous") then
						MarkerNum = "GM-raidMarker" .. marker.textAtlas
					end
				end
				PrintDebug(CreateAtlasMarkup(MarkerNum) .. " Marker Placed")
			end
		end)
	else
	...
	SecureHandlerUnwrapScript(Placer_Button, "PreClick")
...
function WorldMarkerCycler:ShowOptions(state)
	if state then
		self:CreateOptions()
		for i = 1, #Ambrosia.WorldMarkers do
			local checkbox = _G["AMT_Cycler_" .. Ambrosia.WorldMarkers[i].icon .. "_Button"]
			for key, value in pairs(self.db) do
				if string.sub(key, 1, 7) == "Cycler_" then
					if key == "Cycler_" .. Ambrosia.WorldMarkers[i].icon then
...
local function ToggleWorldMarker(self)
	...
	for i = 1, #Ambrosia.WorldMarkers do
		for key, value in pairs(WorldMarkerCycler.db) do
			if string.sub(key, 1, 7) == "Cycler_" then
				if key == "Cycler_" .. Ambrosia.WorldMarkers[i].icon and value then
					tinsert(new_WorldMarkerCycler_Order, Ambrosia.WorldMarkers[i].wmID)
				end
			end
		end
	end
	if #new_WorldMarkerCycler_Order == 1 then
		new_WorldMarkerCycler_Order[2] = new_WorldMarkerCycler_Order[1]
		return
	end
	WorldMarkerCycler.db.WorldMarkerCycler_Order = new_WorldMarkerCycler_Order
	order = WorldMarkerCycler.db.WorldMarkerCycler_Order
...
function WorldMarkerCycler:CreateOptions()
	...
	if not _G["AMT_Cycler_OptionsPane"] then
		f = Ambrosia:CreateOptionsPane("AMT_Cycler_OptionsPane")
		...
		for i = 1, #Ambrosia.WorldMarkers do
			if not _G["WMFrame" .. i] then
				...
				WM_Icon:SetText(CreateAtlasMarkup("GM-raidMarker" .. (#Ambrosia.WorldMarkers + 1 - i), 32, 32))
				WM_Icon:SetPoint("TOP", WM_Frame, "TOP", 0, -8)

				-- Checkbox
				local WM_Button = Ambrosia.CreateCustomCheckbox(
					WM_Frame,
					"AMT_Cycler_" .. Ambrosia.WorldMarkers[i].icon .. "_Button",
					28
				)
				WM_Button:SetPoint("TOP", WM_Icon, "BOTTOM", 0, -8)
				WM_Button.dbKey = "WorldMarkerCyclerSettings.Cycler_" .. Ambrosia.WorldMarkers[i].icon
...
function Ambrosia:WorldMarkerCycler_ToggleConfig()
...
do
	local function EnableModule(state)
		if not WorldMarkerCycler.db then
			WorldMarkerCycler.db = Ambrosia.db.WorldMarkerCyclerSettings
			order = WorldMarkerCycler.db.WorldMarkerCycler_Order
			WorldMarkerCycler:Init()
		end
		...
		local Placer_Button = _G["WorldMarker_Placer"]
		if state then
			Placer_Button:SetAttribute("enableMarkers", Ambrosia.db.WorldMarkerCycler)
		else
			Placer_Button:SetAttribute("enableMarkers", Ambrosia.db.WorldMarkerCycler)
		end
	end
