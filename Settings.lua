...
local function CreateNewFeatureMark(button)
	...
	newTag:SetTexture("Interface/AddOns/Ambrosia/Media/Art/Settings/NewFeatureMark")
...
local function CreateCategoryButton(parent)
	...
	b.Background:SetTexture("Interface/AddOns/Ambrosia/Media/Art/Settings/CategoryButton-NineSlice")
	...
	b.Arrow:SetTexture("Interface/AddOns/Ambrosia/Media/Art/Settings/CollapseExpand")
...
local function CreateOptionToggle(checkbox, onClickFunc)
	if not checkbox.OptionToggle then
		...
		b.Texture:SetTexture("Interface/AddOns/Ambrosia/Media/Button/OptionToggle")
...
local function CreateUI()
	...
	local db = Ambrosia.db
	DB = db
	local settingsOpenTime = db.settingsOpenTime or 0

	local parent = Config
	local showCloseButton = true
	local f = Ambrosia.CreateHeaderFrame(parent, showCloseButton)
	parent.Frame = f
	...
	mask:SetTexture(
		"Interface/AddOns/Ambrosia/Media/Art/Settings/PreviewMask",
	...
	dividerTop:SetTexture("Interface/AddOns/Ambrosia/Media/Frame/Divider_DropShadow_Vertical")
	dividerBottom:SetTexture("Interface/AddOns/Ambrosia/Media/Frame/Divider_DropShadow_Vertical")
	dividerMiddle:SetTexture("Interface/AddOns/Ambrosia/Media/Frame/Divider_DropShadow_Vertical")
	...
	SelectionTexture:SetTexture("Interface/AddOns/Ambrosia/Media/Art/Settings/SelectionTexture")
	...
	for i, data in ipairs(parent.modules) do
		...
		checkbox = Ambrosia.CreateCheckbox(lastCategoryButton.Drawer)
...
function Config:AddModule(moduleData)
	if not moduleData.categoryID then
		...
		PrintDebug(moduleData.name, "No Category")
	end
...
local Ambrosia_SettingsID

if Settings then
	local panel = Config
	local category = Settings.RegisterCanvasLayoutCategory(panel, "Ambrosia")
	Settings.RegisterAddOnCategory(category)
	Ambrosia_SettingsID = category:GetID()
end

-- =========================
-- === Set Slash Command ===
-- =========================
function AMB_DebugCommands(msg)
	if msg == "debug" then
		Ambrosia.db.DebugMode = not Ambrosia.db.DebugMode
		if Ambrosia.db.DebugMode then
			PrintDebug("|cff19ff19Activated|r")
		else
...
SLASH_AMB1 = "/amb"
SlashCmdList["AMB"] = AMB_DebugCommands
